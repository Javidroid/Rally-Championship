...
import java.util.*;
/**
 * Sirve para ordenar la lista de Pilotos por Total de Puntos
 * 
 * @author Javier Florido Cartolano, Eugenia Andújar Sánchez y Carmen Martín Granado
 * @version v1 (30/12/2020)
 */
public class SortByTotalPuntos implements CMPStrategyPiloto
{
    /**
    ...
    public void ordenarPilotos(List <Piloto> pilotos, boolean ASC){
        if(ASC){
            Collections.sort(pilotos, new CMPTotalPuntos());
        }
        else{
            Collections.sort(pilotos, Collections.reverseOrder(new CMPTotalPuntos()));
        }
    }
}
